% Malthe extra data
extraData = load('MaltheNewDataOutward3.mat')
extraDatafsr = extraData.raw_data.open;
extraDatafsr = extraDatafsr.fsr_data;
plot(extraDatafsr)
%%
extraInterval = {[1,1001],[1001,2001],[8000,9000],[9001,10001],[13000,14000],[14001,15001],[21000,22000],[24001,25001]}
cutExtraData = extract_and_plot_intervals(extraDatafsr,extraInterval);
for i = 1:8
    filename = sprintf('dataMaltheExtraDataOutward3%01d.xls',i);
    xlswrite(filename, cutExtraData(:,:,i))
end
%% Malthe new data no load
newMaltheNoLoad = load('MaltheNoLoadNew.mat')
newMaltheNoLoadfsr = newMaltheNoLoad.raw_data.open;
newMaltheNoLoadfsr = newMaltheNoLoadfsr.fsr_data;

plot(newMaltheNoLoadfsr)
%% Malthe new data flexed no load
newIntervalsNoLoad = {[1,5000]}
cutNewMaltheNoLoadfsr = extract_and_plot_intervals(newMaltheNoLoadfsr,newIntervalsNoLoad);

filename = sprintf('dataNewMaltheNoLoadfsrFlexed.xls');
xlswrite(filename, cutNewMaltheNoLoadfsr(:,:,1))

%% Malthe new data Rest no load
newIntervalsNoLoadRest = {[7001,12000]}
cutNewMaltheNoLoadfsrRest = extract_and_plot_intervals(newMaltheNoLoadfsr,newIntervalsNoLoadRest);

filename = sprintf('dataNewMaltheNoLoadfsrRest.xls');
xlswrite(filename, cutNewMaltheNoLoadfsrRest(:,:,1))
%% Malthe new data load 1
newMaltheLoad = load('MaltheLoadNew.mat')
newMaltheLoadfsr = newMaltheLoad.raw_data.open;
newMaltheLoadfsr = newMaltheLoadfsr.fsr_data;

plot(newMaltheLoadfsr)
%% Malthe new data load 1 flexed
newIntervalsLoad = {[1,5000]}
cutNewMaltheLoadfsr = extract_and_plot_intervals(newMaltheLoadfsr,newIntervalsLoad);

filename = sprintf('dataNewMaltheLoadfsrFlexed.xls');
xlswrite(filename, cutNewMaltheLoadfsr(:,:,1))
%% Malthe new data load 1 rest
newIntervalsLoadRest = {[7001,12000]}
cutNewMaltheLoadfsrRest = extract_and_plot_intervals(newMaltheLoadfsr,newIntervalsLoadRest);

filename = sprintf('dataNewMaltheLoadfsrRest.xls');
xlswrite(filename, cutNewMaltheLoadfsrRest(:,:,1))
%% Malthe new data load 2
newMaltheLoad2 = load('MaltheLoad2New.mat')
newMaltheLoad2fsr = newMaltheLoad2.raw_data.open;
newMaltheLoad2fsr = newMaltheLoad2fsr.fsr_data;

plot(newMaltheLoad2fsr)
%% Malthe new data load 2 flexed
newIntervalsLoad2 = {[1,5000]}
cutNewMaltheLoad2fsr = extract_and_plot_intervals(newMaltheLoad2fsr,newIntervalsLoad2);

filename = sprintf('dataNewMaltheLoad2fsrFlexed.xls');
xlswrite(filename, cutNewMaltheLoad2fsr(:,:,1))
%% Malthe new data load 2 rest
newIntervalsLoad2Rest = {[9000,13000]}
cutNewMaltheLoad2fsrRest = extract_and_plot_intervals(newMaltheLoad2fsr,newIntervalsLoad2Rest);

filename = sprintf('dataNewMaltheLoad2fsrRest.xls');
xlswrite(filename, cutNewMaltheLoad2fsrRest(:,:,1))
%%
% Malthe Large Load
