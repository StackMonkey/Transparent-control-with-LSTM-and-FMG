...
AF_gains MR_AF_gains = {0.01, 2.5, 0.001};
AF_gains ML_AF_gains = {0.01, 2.5, 0.001};
float damping_gain_factor = 250;
float base_line_damping = damping_gain_factor;
//Encoder knobRight(MR_PE_A, MR_PE_B);

float assist_val_right = 0;
float assist_val_left = 0;
float last_detected = millis();
float last_detected_a = millis();
...
void setup() {
  ...
  left.LSTMsetup();
	...
	SerialBT.print('M');  // sending character to PS
	SerialBT.print('>');  // sending character to PS
	//Serial.print('M');  // sending character to PS
	//Serial.print('>');  // sending character to
	while (c != 'B') {
		c = MySerial.read();
	}

	MySerial.println('A');  // sending character to PS
	...
	base_line_damping = 250 * MR_AF_gains.inertia_val;
	ML_AF_gains.damping_val = base_line_damping;
	MR_AF_gains.damping_val = base_line_damping;
}
...
void loop() {
	...
	Exo_filter_data[1] =
			(lp_vals.ts * lp_vals.wc * Exo_filter_data[1] + lp_vals.old_vel_MR) / (1 + lp_vals.ts * lp_vals.wc);
	lp_vals.old_vel_MR = Exo_filter_data[1];
	Exo_filter_data[2] =
			(lp_vals.ts * lp_vals.wc * Exo_filter_data[2] + lp_vals.old_cur_MR) / (1 + lp_vals.ts * lp_vals.wc);
	lp_vals.old_cur_MR = Exo_filter_data[2];

	Exo_filter_data[4] =
			(lp_vals.ts * lp_vals.wc * Exo_filter_data[4] + lp_vals.old_vel_ML) / (1 + lp_vals.ts * lp_vals.wc);
	lp_vals.old_vel_ML = Exo_filter_data[4];
	Exo_filter_data[5] =
			(lp_vals.ts * lp_vals.wc * Exo_filter_data[5] + lp_vals.old_cur_ML) / (1 + lp_vals.ts * lp_vals.wc);
	...
	if (control_strategy == 'T') {
		//MR_AF_gains.inertia_val = 0.01; // 0.038 //M
		//ML_AF_gains.inertia_val = 0.01; // 0.038 //M
		//MR_AF_gains.damping_val = 5;     //D
		//ML_AF_gains.damping_val = 5;     //D
		current_time_trajectory = millis();

		if (((current_time_trajectory - starting_time_trajectory) / 1000) <= compeletion_time_trajectory) {
			desired_torque_MR = trajectory_generator(initial_torque_MR_trajectory, final_torque_MR_trajectory,
			                                         (current_time_trajectory - starting_time_trajectory) / 1000,
			                                         compeletion_time_trajectory);
			desired_torque_ML = trajectory_generator(initial_torque_ML_trajectory, final_torque_ML_trajectory,
			                                         (current_time_trajectory - starting_time_trajectory) / 1000,
			                                         compeletion_time_trajectory);
			adaptive_damping = (desired_torque_MR / 10) * 3;
			MR_AF_gains.damping_val = base_line_damping + adaptive_damping;     //D
			ML_AF_gains.damping_val = base_line_damping + adaptive_damping;     //D
			difference_val = (LC_filter_data[0] - desired_torque_MR);
			desired_velocity_MR = admittance_filter_MR(difference_val);
			difference_val = (LC_filter_data[3] - desired_torque_ML);
			desired_velocity_ML = admittance_filter_ML(difference_val);
		} else {
			desired_torque_MR = final_torque_MR_trajectory;
			desired_torque_ML = final_torque_ML_trajectory;
			adaptive_damping = (desired_torque_MR / 10) * 3;
			MR_AF_gains.damping_val = base_line_damping + adaptive_damping;     //D
			ML_AF_gains.damping_val = base_line_damping + adaptive_damping;     //D
			difference_val = (LC_filter_data[0] - desired_torque_MR);
	...
	if (control_strategy == 'A') {
    	...
    	float rtorque = right.Torque(Exo_filter_data[0]*3.142/180,Exo_filter_data[1],0,0.002);
   		float ltorque = left.Torque(Exo_filter_data[3]*3.142/180,Exo_filter_data[4],0,0.002);
      float scaledrtorque = right.Scaling_transformation(right.diffTorque(rtorque),-1,1,-0.00049565,0.00049565);
      float scaledltorque = left.Scaling_transformation(left.diffTorque(ltorque),-1,1,-0.00049565,0.00049565);
    	float rtorquePre = right.Inverse_Scaling_transformation(right.LSTMpredict(scaledrtorque),-1,1,-0.00049565,0.00049565);
    	float ltorquePre = left.Inverse_Scaling_transformation(left.LSTMpredict(scaledltorque),-1,1,-0.00049565,0.00049565);
      float invrtorquePre = right.invDiffTorque(rtorquePre);
      float invltorquePre = left.invDiffTorque(ltorquePre);
		desired_velocity_MR = admittance_filter_MR(rtorque);// + assist_val_right); LC_filter_data[0]
		desired_velocity_ML = admittance_filter_ML(ltorque);// + assist_val_left); LC_filter_data[3]
	}
	...
	if (received_command == 't') {
		...
		if (Exo_filter_data[0] > 40 && Exo_filter_data[3] > 40) {
			control_strategy = 'T';
			control_status = 'C';
			initial_torque_MR_trajectory = 0;//LC_filter_data[0];
			initial_torque_ML_trajectory = 0;//LC_filter_data[3];
			final_torque_MR_trajectory = max_assist_value;
			final_torque_ML_trajectory = max_assist_value;
	...
	if (received_command == 'A') {
		control_strategy = 'A';
		received_command = 'n';
		/*if ((millis() - last_detected_a) > 2000)
		{
		  last_detected_a = millis();
		  control_strategy = 'A';
		  control_status = 'C';
		  initial_torque_MR_trajectory = max_assist_value;//LC_filter_data[0];
		  initial_torque_ML_trajectory = max_assist_value;//LC_filter_data[3];
		  final_torque_MR_trajectory = 0;
		  final_torque_ML_trajectory = 0;
		  starting_time_trajectory = millis();
		}
		received_command = 'n';*/
	}
		...
		if (motor_actuation_loop >= (motor_actuation_time / control_loop_time)) {
			...
			if (control_strategy == 'F') {
				...
				if (((current_time_trajectory - starting_time_trajectory) / 1000) <= compeletion_time_trajectory_f) {
					desired_position_MR = trajectory_generator(initial_position_MR_trajectory,
					                                           final_position_MR_trajectory,
					                                           (current_time_trajectory - starting_time_trajectory) /
					                                           1000, compeletion_time_trajectory_f);
					PID_control((desired_position_MR - Exo_filter_data[0]), &MR_pos_gains);
					current_control(MR_pos_gains.output, MR_dir, MR_pwm_channel);
					desired_position_ML = trajectory_generator(initial_position_ML_trajectory,
					                                           final_position_ML_trajectory,
					                                           (current_time_trajectory - starting_time_trajectory) /
					                                           1000, compeletion_time_trajectory_f);
					...
					initial_torque_MR_trajectory = 0;
					final_torque_MR_trajectory = 0;
					initial_torque_ML_trajectory = 0;
					final_torque_ML_trajectory = 0;
				}
			}
...
void readExo(float *Exo_readings) {
	...
	Exo_readings[1] = analogRead(MR_vel) * m_velocity_in + c_velocity_in + 0.6; // velocity
	...
	Exo_readings[4] = analogRead(ML_vel) * m_velocity_in + c_velocity_in + 0.6; // velocity
...
void readLC(float *LC) {
	...
	LC[0] = -1 * read_digit() * 0.18;
	delayMicroseconds(10);

	MySerial.print('2');
	delayMicroseconds(10);
	LC[1] = read_digit() * 0.18;
	delayMicroseconds(10);

	MySerial.print('3');
	delayMicroseconds(10);
	LC[2] = read_digit() * 0.18;
	delayMicroseconds(10);

	MySerial.print('4');
	delayMicroseconds(10);
	LC[4] = read_digit() * 0.18;
	delayMicroseconds(10);

	MySerial.print('5');
	delayMicroseconds(10);
	LC[3] = -read_digit() * 0.18;
	delayMicroseconds(10);

	MySerial.print('6');
	delayMicroseconds(10);
	LC[5] = read_digit() * 0.18;
	delayMicroseconds(10);
	MySerial.print('t');
}
		...
		if (SerialBT.available()) {
			...
			if (received_command != 'T') {
				...
				if (mc_sel == "raf") {
					...
					damping_gain_factor = MR_AF_gains.damping_val;
					gains_sel = gains_sel.substring(comma_index + 1);
					comma_index = gains_sel.indexOf(',', 0);
					gains_val = gains_sel.substring(0, comma_index);
					MR_AF_gains.stiffness_val = gains_val.toFloat();
					base_line_damping = damping_gain_factor * MR_AF_gains.inertia_val;
				}
				...
				if (mc_sel == "laf") {
					...
					damping_gain_factor = ML_AF_gains.damping_val;
					gains_sel = gains_sel.substring(comma_index + 1);
					comma_index = gains_sel.indexOf(',', 0);
					gains_val = gains_sel.substring(0, comma_index);
					ML_AF_gains.stiffness_val = gains_val.toFloat();
					base_line_damping = damping_gain_factor * ML_AF_gains.inertia_val;
				}
				if (mc_sel == "rav") {
					comma_index = gains_sel.indexOf(',', 0);
					gains_val = gains_sel.substring(0, comma_index);
